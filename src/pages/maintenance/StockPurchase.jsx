...
export default function StockPurchase() {
  ...
  const [stockHeader, setStockHeader] = useState({
    supplier_name: "", delivery_receipt: "", order_date: null, delivery_date: null, remarks: "",
  });
  ...
  const addRow = () => {
    setStockDetails([
      ...stockDetails,
      { product_id: "", sku: "", quantity: "", price_per_unit: "", cost_per_unit: "", discount: "0", total_purchase_cost: "", big_cost: "", big_price: "", unit: "", big_unit: "", big_qty: "", big_conversion: "1", small_conversion: "", }
    ]);
  };

  const deleteRow = (index) => {
    const updated = stockDetails.filter((_, i) => i !== index);
    setStockDetails(updated);
  };
  ...
  async function handleCreate(e) {
    ...
    if (
      !stockDetails.length || stockDetails.some(item => !item.product_id || !item.big_unit || !item.big_qty || !item.big_conversion || !item.big_cost || !item.big_price ||
    ...
    const header = await fetch("/api/stockheader", {
      method: "post",
      headers: {
        "Content-Type": "application/json",
        "Accept": "application/json",
    ...
    const headerId = dataheader.header_id;

    // 3. Add this ID to each stock detail
    const stockDetailsId = stockDetails.map(item => ({
      ...item,
        header_id: headerId,
    }));

    const details = await fetch("/api/stockdetails", {
      method: "post",
      headers: {
        "Content-Type": "application/json",
        "Accept": "application/json",
      ...
      body: JSON.stringify({ stockDetails: stockDetailsId }),
    });
    ...
    if (dataheader.errors) {
      ...
      setStockDetails([
        { product_id: "", sku: "",
          quantity: "", cost_per_unit: "", price_per_unit: "",
          discount: "", total_purchase_cost: "" }
      ]);
    }
    setLoading(false);
    stopLoading();
              ...
              <div className='border p-2 bg-gray-100'>
                <div className="flex flex-row py-2 p-4">
                  ...
                  <div className="flex mb-3">
                    <button className="h-fit flex flex-row primary-btn py-3 px-5 text-sm tracking-wider font-semibold rounded-md text-white bg-gray-800 hover:bg-gray-600 focus:outline-none"><FaSave size={20} className="mr-2"/>Add&nbsp;Stock</button>
                  </div> 
                </div>
                <hr />
                        ...
                        <caption className="text-left caption-top dark:text-gray-800">
                          ...
                          <tbody className="text-sm text-center bg-gray-50 dark:bg-gray-100 dark:text-gray-800 border-b-2 border-gray-300">
                            {stockDetails.map((row, index) => (
                              <>
                            <tr className="text-xs bg-white border-b dark:bg-gray-50 dark:border-gray-300" key={index}>  
                              <td className="px-2">
                              ...
                              {/* <td className="px-3 py-3">
                                <input type="text" placeholder="PPU" name={`stockDetails[${index}][price_per_unit]`} value={row.price_per_unit} onChange={(e) => handleHardware(index, "price_per_unit", e.target.value)} className="register-link py-2 px-2 w-full border rounded-md text-right" />
                              </td> */}
                              {/* <td className="px-3 py-3">
                                <input type="text" placeholder="Quantity" name={`stockDetails[${index}][quantity]`} value={row.quantity} onChange={(e) => handleHardware(index, "quantity", e.target.value)} className="register-link py-2 px-2 w-full border rounded-md text-center" />
                              </td> */}
                              <td className="px-2 py-3">
                                <div className="relative">
                                  <input type="text" name={`stockDetails[${index}][discount]`} value={row.discount} onChange={(e) => handleHardware(index, "discount", e.target.value)} className="register-link py-2 px-6 w-full border rounded-md text-right" max="100" min="0" />
                              ...
                              {/* {expandedRows.includes(index) && (
                                <tr className="bg-gray-50 text-xs border-b dark:border-gray-300">
                                  <td></td>
                                  <td className="p-3">
                                    <input type="text" placeholder="Common Name" name={`stockDetails[${index}][common_name]`} value={row.common_name} onChange={(e) => handleHardware(index, "common_name", e.target.value)} className="register-link py-2 px-4 w-full border rounded-md text-left" />
                                  </td>
                                  <td className="p-3">
                                    <input type="text" placeholder="Big Qty" name={`stockDetails[${index}][big_quantity]`} value={row.big_quantity} onChange={(e) => handleHardware(index, "big_quantity", e.target.value)} className="register-link py-2 px-4 w-full border rounded-md text-center" />
                                  </td>
                                  <td colSpan={2} className="p-3">
                                    <input type="text" placeholder="Big Unit" name={`stockDetails[${index}][big_unit]`} value={row.big_unit} onChange={(e) => handleHardware(index, "big_unit", e.target.value)} className="register-link py-2 px-4 w-full border rounded-md text-center" />
                                  </td>
                                  <td colSpan={2} className="p-3">
                                    <input type="text" placeholder="Big Amount" name={`stockDetails[${index}][big_amount]`} value={row.big_amount} onChange={(e) => handleHardware(index, "big_amount", e.target.value)} className="register-link py-2 px-4 w-full border rounded-md text-right" />
                                  </td>
                                  <td colSpan={2}>&nbsp;</td>
                                </tr>
                              )} */}
                          ...
                          <tr>
                            ...
                            <td className="px-3 py-3">
                              <input
                                ...
                                value={stockDetails[selectedIndex]?.big_conversion ?? ""}
